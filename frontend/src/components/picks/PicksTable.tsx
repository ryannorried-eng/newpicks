          ...
          {picks.map((pick) => (
            <Fragment key={pick.id}>
              <tr key={pick.id} className="cursor-pointer border-t border-gray-800 hover:bg-gray-800/40" onClick={() => setExpandedId(expandedId === pick.id ? null : pick.id)}>
                <td className="p-3"><div className="flex items-center gap-2"><SportIcon sport={pick.sport_key} />{pick.sport_key}</div></td><td>{pick.away_team} @ {pick.home_team}</td><td>{new Date(pick.commence_time).toLocaleString()}</td><td>{pick.market}</td><td>{pick.side} {pick.line ?? ""}</td><td className="font-mono">{pick.odds_american}</td><td>{pick.best_book}</td><td className="text-emerald-400">{(pick.ev_pct * 100).toFixed(1)}%</td><td><ConfidenceBadge tier={pick.confidence_tier} /></td><td>{pick.composite_score.toFixed(2)}</td><td>{pick.suggested_kelly_fraction.toFixed(3)}</td>
              </tr>
